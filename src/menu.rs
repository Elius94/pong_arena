...
fn render(out: &mut String, s: &State, cols: usize, rows: usize) {
    ...
    // ── Nickname (solo nel menu principale) ──
    if matches!(s.screen, Screen::Main) {
        let nick_line = format!("Nickname: {}", if s.nickname.is_empty() { "(vuoto)" } else { &s.nickname });
        let col = centered_col(cols, &nick_line);
        out.push_str(&mv(row, col));
        out.push_str(&fg(DIM));
        out.push_str(&nick_line);
        row += 2;
    }
    ...
    for i in 0..s.num_items() {
        // Nel menu principale, skip Nickname (già mostrato sopra)
        if matches!(s.screen, Screen::Main) && i == 2 {
            continue;
        }
